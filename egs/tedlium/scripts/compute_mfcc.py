...
def compute_mfcc(hdf5_path: str, segment_dict, wav_dict):
    ...
    with h5py.File(hdf5_path, 'w') as hf:
        for wav_id in segment_dict:
            ...
            for utterance_id, from_, to_ in segment_dict[wav_id]:
                ...
                hf.create_dataset(utterance_id, data=mfcc)

    return f"Done computing mfcc features - {hdf5_path}"
...
def main(args):
    print("Compute mfcc features for", args.data)
    ...
    # Compute mfcc features in parallel
    processes = []
    for i in range(args.nprocs):
        processes.append(
            mp.Process(
                target=compute_mfcc, 
                args=(os.path.join(args.data, f"data/mfcc{i}.hdf5"), segment_dict_per_proc[i], wav_dict)
            )
        )
        processes[-1].start()
    ...
    with open(os.path.join(args.data, "mfcc_utterance_mapping"), 'w') as f:
        for i in range(args.nprocs):
            segment_dict = segment_dict_per_proc[i]
            for wav_id in segment_dict:
                for utterance_id, _, _ in segment_dict[wav_id]:
                    # get absolute path to the hdf5 file
                    hdf5_path = os.path.join(args.data, f"data/mfcc_{i}.hdf5")
