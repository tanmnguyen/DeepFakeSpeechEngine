...
# Modifed the code from make_spectrogram.sh to compute mel filter bank features
...
Options:
  --spectrogram-config <config-file>          # config passed to compute-spectrogram-feats.
...
if [ -f $data/spectrogram_feats.scp ]; then
  mkdir -p $data/.backup
  echo "$0: moving $data/spectrogram_feats.scp to $data/.backup"
  mv $data/spectrogram_feats.scp $data/.backup
fi
...
for n in $(seq $nj); do
  ...
  utils/create_data_link.pl $spectrogramdir/raw_spectrogram_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_spectrogram_${name}.JOB.log \
    ...
    compute-spectrogram-feats $vtln_opts $write_utt2dur_opt --verbose=2 \
      --config=$spectrogram_config ark:- ark:- \| \
  ...
  $cmd JOB=1:$nj $logdir/make_spectrogram_${name}.JOB.log \
    compute-spectrogram-feats $vtln_opts $write_utt2dur_opt --verbose=2 \
      --config=$spectrogram_config scp,p:$logdir/wav_${name}.JOB.scp ark:- \| \
...
if [ -f $logdir/.error.$name ]; then
  echo "$0: Error producing spectrogram features for $name:"
...
done > $data/spectrogram_feats.scp || exit 1
...
# Store frame_shift and spectrogram_config along with features.
frame_shift=$(perl -ne 'if (/^--frame-shift=(\d+)/) {
                          printf "%.3f", 0.001 * $1; exit; }' $spectrogram_config)
echo ${frame_shift:-'0.01'} > $data/frame_shift
mkdir -p $data/conf && cp $spectrogram_config $data/conf/spectrogram.conf || exit 1

rm $logdir/wav_${name}.*.scp  $logdir/segments.* \
   $logdir/utt2num_frames.* $logdir/utt2dur.* 2>/dev/null

nf=$(wc -l < $data/spectrogram_feats.scp)
...
echo "$0: Succeeded creating spectrogram features for $name"
